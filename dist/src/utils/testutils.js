...
exports.createAccount = exports.userOpsWithoutAgg = exports.simulationResultWithAggregationCatch = exports.simulationResultCatch = exports.objdump = exports.decodeRevertReason = exports.rethrow = exports.getAccountAddress = exports.getAccountInitCode = exports.callDataCost = exports.tonumber = exports.tostr = exports.FIVE_ETH = exports.TWO_ETH = exports.ONE_ETH = exports.HashZero = exports.AddressZero = void 0;
...
function callDataCost(data) {
    return ethers_1.ethers.utils.arrayify(data)
        .map(x => x === 0 ? 4 : 16)
        .reduce((sum, x) => sum + x);
}
...
function decodeRevertReason(data, nullIfNoMatch = true) {
    ...
    if (methodSig === '0x08c379a0') {
        ...
        const [opindex, paymaster, msg] = ethers_1.ethers.utils.defaultAbiCoder.decode(['uint256', 'address', 'string'], dataParams);
        // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
        return `FailedOp(${opindex}, ${paymaster !== exports.AddressZero ? paymaster : 'none'}, ${msg})`;
    }
 ...
 */
function simulationResultCatch(e) {
    if (e.errorName !== 'ValidationResult') {
        throw e;
    }
    return e.errorArgs;
}
exports.simulationResultCatch = simulationResultCatch;
/**
 ...
 */
function simulationResultWithAggregationCatch(e) {
    if (e.errorName !== 'ValidationResultWithAggregation') {
        throw e;
    }
    return e.errorArgs;
}
exports.simulationResultWithAggregationCatch = simulationResultWithAggregationCatch;
...
async function createAccount(ethersSigner, accountOwner, entryPoint, _factory) {
    const accountFactory = _factory ?? await new typechain_types_1.SimpleAccountFactory__factory(ethersSigner).deploy(entryPoint);
    const implementation = await accountFactory.accountImplementation();
    await accountFactory.createAccount(accountOwner, 0);
    const accountAddress = await accountFactory.getAddress(accountOwner, 0);
    const proxy = typechain_types_1.SimpleAccount__factory.connect(accountAddress, ethersSigner);
    return {
        implementation,
        accountFactory,
        proxy
